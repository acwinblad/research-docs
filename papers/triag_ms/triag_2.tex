...
\Blue{(0) brief history of MFs in twenty years}
For the past twenty years Majorana zero-modes (MZMs) in condensed matter systems have been highly sought after due to both its fault-tolerant topological protection and non-Abelian exchange statistics for quantum computating \ref{}.
\Blue{(1) Designs based on the kitaev chains and experimental evidence.}
MZMs were originally proposed to be found in topological 2D \textit{p}-wave superconducting vortices and at the ends of a topological 1D \textit{p}-wave superconductor \ref{}.
...
Such systems make use of \textit{s}-wave superconductors in proximity to various heterostructures: topological insulator \ref{} or Josephson junction with in plane magnetic field \ref{}, 2D electron gas (2DEG) with Rashba spin-orbit coupling (SOC) and ferromagnet \ref{}, ferromagnetic atomic spin chains or spin-helical order \ref{}, and Quantum anomalous Hall insulator (QAHI) \ref{}.
The past decade has seen strides in potential measurements of MZMs, most of these focus on one end of the wire \ref{} looking at zero-bias conductance peak (ZBP) curves and more recently both ends of a wire \ref{}.
A new potential measurement may soon help confirm the identity of MZMs using a quantum dot and crossed Andreev reflections (CAR) in a Josephson junction \ref{}.
\Blue{(2) Geometries different from chain, quasi-1D, 2D options}
...
Others have pointed this is not a simple task and proposed quasi-1D systems like a cross Josephson junction \ref{} or scissor cuts on a QAHI \ref{} to get around the difficult engineering of 1D wire networks.
We propose an alternative structure by using triangular superconducting islands, which can be easily produced with epitaxial growth \ref{}.
\Blue{(3) Supercurrent and TSC}
...
We show one can get exact MZMs in a simple three site triangular lattice.
...
It has been previously shown that a constant vector potential can induce a topological phase transition for hybrid effective $p$-wave and spinless $p$-wave superconducting chain \cite{romitoManipulatingMajoranaFermions2012,takasanSupercurrentinducedTopologicalPhase2022}, and other types of superconductors.
